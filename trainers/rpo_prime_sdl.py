...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        img_org_f = img_f[:, 0]
        img_rpo_f = img_rpo_f / img_rpo_f.norm(dim=-1, keepdim=True) #(batch, model, emb)
        ...
        mix_logit = mix_logit.reshape(-1, batch_size, self.num_classes) #((K+1)*(K+1), batch, class)
        #half 추가 - first half에는 cov loss 안하고, sdl만 주는 세팅
        
        first_half_logit = self.logit_scale.exp() * torch.einsum('pbe,Pec->pPbc',torch.cat([img_rpo_f[:, :self.K1//2], img_org_f.unsqueeze(1)], dim=1).permute(1, 0, 2), torch.cat([text_rpo_f[:, :self.K1//2], text_org_f.unsqueeze(1)], dim=1).permute(1,2,0)) #eos token 포함
        #img permute 후 (model, batch, emb) / text permute 후 (model, emb, class)
        first_half_logit = first_half_logit.reshape(-1, batch_size, self.num_classes).to(device) #(K+1)*(K+1), batch, class
        ...
        if self.prompt_learner.training:
            ...
            #                           label.reshape([1, -1]).repeat([self.num_models, 1]).reshape([-1, 1]).squeeze(),
            #                           reduction='none', label_smoothing=0.1)
            # #print("ce_loss:",ce_loss.size()) #(K+1)*(K+1)*batch
            # ce_loss = ce_loss.reshape([self.num_models, batch_size]) #(models, batch)

            # masking = (torch.rand_like(ce_loss) > 0.8).type(self.dtype) # -> 각 샘플별로 모델 masking 
            # loss = ce_loss * masking #+ kl_loss #* (1 - masking)
            #loss = F.cross_entropy(mix_logit.mean(0), label)
            ...
            cov_loss = second_img_cov_loss + second_text_cov_loss
            """sdl logit"""
            #기존) 
            #logits = mix_logit.mean(0) #(batch, class) -> 20~
            logits = first_half_logit.mean(0)
            token_logits = self.logit_scale.exp() * img_org_f @ text_org_f.t() #(batch, class)
            
            loss_sdl = F.kl_div(logits,
                            F.log_softmax(token_logits / 1, dim=1),
                            reduction='sum', 
                            log_target=True) * (1 * 1) / logits.numel()

            
            return ce_loss + self.cfg.TRAINER.RPO.cov_loss * cov_loss + self.cfg.TRAINER.RPO.sdl_loss * loss_sdl * 0.1
            #return loss  +  500.0 * (img_cov_loss + text_cov_loss) + self.cfg.TRAINER.RPO.sdl_loss * loss_sdl * 0.1#(self.num_models * batch_size)
            
            #변경) - ce loss를 구할때와 같은 모델들만을 사용하게끔 masking하려는게 목적 !
            # mix_logit_for_sdl = mix_logit * masking.unsqueeze(-1) #masking
            # logits_sdl = torch.empty((batch_size, self.num_classes)).to(device)
            # for i in range(self.num_classes):
            #     logits_sdl[:,i] = torch.sum(mix_logit_for_sdl[:,:,i], dim=0) / torch.sum(masking,dim=0)
           
            # token_logits = self.logit_scale.exp() * img_org_f @ text_org_f.t() #(batch, class) #20~
            
            # loss_sdl = F.kl_div(logits_sdl,
            #                 F.log_softmax(token_logits / 1, dim=1),
            #                 reduction='sum', 
            #                 log_target=True) * (1 * 1) / logits_sdl.numel()

            # return torch.sum(loss) / torch.sum(masking) + self.cfg.TRAINER.RPO.sdl_loss * loss_sdl * 0.1#(self.num_models * batch_size)
