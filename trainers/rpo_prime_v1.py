...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        #     mix_logit.append(self.logit_scale.exp() * img_all_f[:, i, :] @ text_all_f[:, i, :].t())
        # mix_logit = torch.stack(mix_logit, 0)
        ...
        # ##
        # cls_first_img_rpo_f = torch.concat([img_org_f.unsqueeze(1), img_rpo_f[:,:self.K1//2]], dim=1)
        # cls_first_text_rpo_f = torch.concat([text_org_f.unsqueeze(1), text_rpo_f[:,:self.K1//2]], dim=1)
        # cls_second_img_rpo_f = torch.concat([img_org_f.unsqueeze(1), img_rpo_f[:,self.K1//2:]], dim=1)
        # cls_second_text_rpo_f = torch.concat([text_org_f.unsqueeze(1), text_rpo_f[:,self.K1//2:]], dim=1)
        # ##
        ...
        if self.prompt_learner.training:
            ...
            #                           label.reshape([1, -1]).repeat([self.num_models, 1]).reshape([-1, 1]).squeeze(),
                                    ...
                                    # )
            # ce_loss = F.cross_entropy(mix_logit.reshape([-1, self.num_classes]),
            #                           label.reshape([1, -1]).repeat([self.K1, 1]).reshape([-1, 1]).squeeze(),
            #                         )
            # ce_loss = ce_loss.reshape([self.num_models, batch_size])

            # masking = (torch.rand_like(ce_loss) > 0.8).to(device)
            # loss = ce_loss *  masking
            # ce_loss =  F.cross_entropy(mix_logit.mean(0), label) 
            ce_loss = F.cross_entropy(first_half_logit.mean(0), label) + F.cross_entropy(second_half_logit.mean(0), label)
            # return torch.sum(loss) / torch.sum(masking) +  500.0 * (img_cov_loss + text_cov_loss)
            ...
            # # off_diag = img_rpo_cov.pow(2).sum() - img_rpo_cov.pow(2).diag().sum()
            # # return ce_loss  +  self.cfg.TRAINER.RPO.cov_loss * cov_loss
            # # return torch.sum(loss) / torch.sum(masking)+  self.cfg.TRAINER.RPO.cov_loss * cov_loss
            return ce_loss +  self.cfg.TRAINER.RPO.cov_loss * cov_loss            
        #####
        # inference -> ensemble
        return F.softmax(mix_logit, -1).mean(0)
        # return F.softmax(first_half_logit, -1).mean(0)
        # return F.softmax(second_half_logit, -1).mean(0)
        
         # return  F.softmax(mix_logit.mean(0), -1)  #F.softmax(first_half_logit.mean(0), -1) # F.softmax(second_half_logit.mean(0), -1) 



@TRAINER_REGISTRY.register()
class RPO_prime_v1(TrainerX):
